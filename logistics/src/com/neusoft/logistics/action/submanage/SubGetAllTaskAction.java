 ...
 *
 */
public class SubGetAllTaskAction extends DispatchAction{
	...
	public ActionForward doSearch(ActionMapping mapping, ActionForm form,
	    ...
	    //调用服务层得到所有任务单的json信息
	    if(requredatesearch == null && tasktypesearch==null && taskstatesearch == null){
	    	response.getWriter().print(subGetAllTaskService.getSubAllTasks(substation));
	    }else{
	    	response.getWriter().print(subGetAllTaskService.getSubAllTasks(substation, requredatesearch, tasktypesearch, taskstatesearch));
	    }
		return null;
	}
}
