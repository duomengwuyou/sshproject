...
CenterwarehouseInto=function(node){

    //分页每页显示数量	 
	var pageSize = 12;
	//指定列参数
 	var CenterwarehouseIntoFields = ["id","returnProductOrderId","productName","productQuantity"];
	CenterwarehouseIntoStore=new Ext.data.GroupingStore({
           proxy:new Ext.data.HttpProxy({
                url:"centerwarehouseInto.do?actionType=doSearch",
                method:"POST"
           }),
           reader:new Ext.data.JsonReader({
                fields:CenterwarehouseIntoFields,
                root:"data",
                id:"id",
	...
	//alert("测试1");
    //-----------------------右键菜单
    var CenterwarehouseIntoRightClick=new Ext.menu.Menu({
        items:[{
            text: '刷新信息',
            iconCls:'addicon',
            handler:function(){ 
              CenterwarehouseIntoStore.reload();
            }
        },{
            text:'查询任务单信息',
            iconCls:'editicon',
            ...
            text:'中心库房退货入库',
            iconCls:'editicon',
            handler:SureCenterwarehouseInto
        }]
 	});
    //----------------------------------------------------定义grid
	var CenterwarehouseIntoGrid = new Ext.grid.EditorGridPanel({
	    id:"CenterwarehouseIntoGrid",
		store:CenterwarehouseIntoStore,
		sm:sm,
		cm:cm,
		loadMask:true,
		stripeRows:true, 
		...
		new Ext.Toolbar.TextItem("<font color=red>退货单号</font>"),{
		  ...
		  //  tpl: '<tpl for="."><div ext:qtip="{roomTypeName}. {typeRemark}" class="x-combo-list-item">{roomTypeName}</div></tpl>',
			...
			handler:SubstationReturnManage
		},"","-",
		{
			text:"刷新信息",
			//默认样式为按下
...
//             },
             "rowcontextmenu":function(grid,rowIndex,e){
                e.stopEvent();
                CenterwarehouseIntoRightClick.showAt(e.getXY());
             }
		}
	});
    GridMain(node,CenterwarehouseIntoGrid,"roomicons");
}


///--------------------------搜索验货单/分发单信息窗口
SubstationReturnManage = function(){
	SubstationReturn();
}
SureCenterwarehouseInto = function(){
	var row = Ext.getCmp("CenterwarehouseIntoGrid").getSelectionModel().getSelections();
	if(row.length == 0){
		Ext.Msg.alert("提示信息","请您至少选择一个!");
	}else{
		Ext.Msg.alert("提示信息","中心库房退货入库处理!");
