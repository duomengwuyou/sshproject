...
// //------------------------------------------------------查询入住信息
// 搜索主窗体中的内容组件
var SearchReturnTaskOrderContent = new Ext.form.FormPanel({
			width : 375,
			height : 600,
			plain : true,
			layout : "form",
			...
			items : [{
				id : "searchContentReturnTaskOrder",
				name : "searchContentReturnTaskOrder",
...
// 搜索的主窗体
var SearchReturnTaskOrderWin = new Ext.Window({
			title : "查询任务单信息",
			width : 410,
			height : 120,
			plain : true,
			...
			items : [SearchReturnTaskOrderContent],
			listeners : {
				"show" : function() {
					// 当window show事件发生时清空一下表单
					SearchReturnTaskOrderContent.getForm().reset();
				}
			},
			buttons : [{
						text : "搜索",
						minWidth : 70,
						handler : function() {
							if (SearchReturnTaskOrderContent.getForm()
									.isValid()) {
								// 弹出效果
								Ext.MessageBox.show({
											msg : '正在搜索，请稍等...',
						...
						minWidth : 70,
						handler : function() {
							SearchReturnTaskOrderWin.hide();
						}
					}]
		});
SearchReturnTaskOrder = function() {
