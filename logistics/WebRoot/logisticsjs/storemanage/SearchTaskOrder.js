...
// //------------------------------------------------------查询入住信息
// 搜索主窗体中的内容组件
var SearchTaskOrderContent = new Ext.form.FormPanel({
			width : 375,
			height : 600,
			plain : true,
			layout : "form",
			...
			items : [{
				id : "searchContentTaskOrder",
				name : "searchContentTaskOrder",
...
// 搜索的主窗体
var SearchTaskOrderWin = new Ext.Window({
			title : "查询任务单信息",
			width : 410,
			height : 120,
			plain : true,
			...
			items : [SearchTaskOrderContent],
			listeners : {
				"show" : function() {
					// 当window show事件发生时清空一下表单
					SearchTaskOrderContent.getForm().reset();
				}
			},
			buttons : [{
						text : "搜索",
						minWidth : 70,
						handler : function() {
							if (SearchTaskOrderContent.getForm().isValid()) {
								// 弹出效果
								Ext.MessageBox.show({
											msg : '正在搜索，请稍等...',
											progressText : '搜索中...',
						...
						minWidth : 70,
						handler : function() {
							SearchTaskOrderWin.hide();
						}
					}]
		});
SearchTaskOrder = function() {
